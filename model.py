...
class MultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.w_q = nn.Linear(config.cq_dim, sum(self.q_dims), bias=config.bias)
    ...
    def forward(self, x):
        ...
        # split heads and create query by cat query and query rope
        q = torch.cat([
            qc.view(
                B, T, self.n_head, C // self.n_head
            ).transpose(1, 2),  # (B, nh, T, hs) Split heads
            qr.view(
                B, T, self.n_head, qr.size(-1) // self.n_head
            ).transpose(1, 2)  # (B, nh, T, hs) Split heads
        ], dim=-1)

        # create key by cat key and key rope (same k rope for each head)
        k = torch.cat([
            kc.view(B, T, self.n_head, C // self.n_head).transpose(1, 2),
            kr.unsqueeze(1).repeat(1, self.n_head, 1, 1)  # kr is repeated
        ], dim=-1)

        # Split values into heads
        v = vc.view(B, T, self.n_head, C // self.n_head).transpose(1, 2)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = CausalSelfAttention(
            config) if not config.use_mhla else MultiHeadLatentAttention(config)
